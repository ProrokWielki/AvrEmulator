...
mod tests {
    ...
    fn test_process() {
        ...
        expected_registers.set_register((destination_register * 2) as usize, data);
        expected_registers.set_register(((destination_register * 2) + 1) as usize, data + 1);
        expected_registers.set_pc(1);

        let movw = MOVW::new(0xe000 | destination_register << 4 | source_register);
